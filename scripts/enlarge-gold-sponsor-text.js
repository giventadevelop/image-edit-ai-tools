/**
 * Only change: enlarge + Onam-floral decorate "Best compliments from our Gold Sponsor" strip.
 * Everything else stays pixel-identical; resolution unchanged.
 */
...
/** Mini marigold dots for garland along top/bottom */
function garlandDots(y, W, count) {
  let s = '';
  const margin = Math.round(W * 0.22);
  const span = W - margin * 2;
  for (let i = 0; i < count; i++) {
    const x = margin + Math.round((span * i) / (count - 1));
    const colors = ['#e85d04', '#f48c06', '#dc2f02', '#e9c46a', '#ffba08'];
...
async function main() {
  const targets = [
    path.join(dir, 'parsippany_onam_gold_sponsor_anish_landscape.png'),
    path.join(dir, 'parsippany_onam_gold_sponsor_anish_landscape_50pct.png'),
    path.join(dir, 'sponsors_landscape', 'parsippany_onam_gold_sponsor_anish_landscape_50pct.png'),
    path.join(dir, 'sponsors_landscape', 'parsippany_onam_gold_sponsor_roy_mathew_landscape.png'),
  ];
  ...
  // Bigger than previous ~187; side room kept for florals
  const fontSize = Math.round(W * 0.041); // ~226px

  // Side floral clusters — outer margins only
  const leftCluster =
  ...
  const svg =
    ...
    '<rect x="0" y="0" width="' +
    W +
    '" height="6" fill="url(#gold)"/>' +
    '<rect x="0" y="' +
    (stripH - 6) +
    '" width="' +
    W +
    '" height="6" fill="url(#gold)"/>' +
    // marigold garland dots under top / above bottom gold rules
    garlandDots(20, W, 21) +
    garlandDots(stripH - 20, W, 21) +
    leftCluster +
    rightCluster +
    // soft shadow + larger main text
    '<text x="50%" y="' +
    (cy + 8) +
    '" text-anchor="middle" dominant-baseline="middle" ' +
    'font-family="Georgia, Times New Roman, Times, serif" font-size="' +
    fontSize +
    '" font-weight="700" fill="#8a6a1a" opacity="0.25">' +
    'Best compliments from our Gold Sponsor</text>' +
    '<text x="50%" y="' +
    cy +
    '" text-anchor="middle" dominant-baseline="middle" ' +
    'font-family="Georgia, Times New Roman, Times, serif" font-size="' +
  ...
  for (const t of targets) {
    ...
    await sharp(outBuf).png().toFile(t);
    console.log('updated', path.relative(dir, t));
  }
  ...
  await sharp(outBuf)
    ...
    .png()
    .toFile(path.join(dir, '_tmp_gold_strip_check.png'));
    ...
    JSON.stringify(
      {
        ...
        stripH,
        fontSize,
        change: 'Bigger text + side marigold/hibiscus clusters + garland dots',
      },
      null,
      2
    )
